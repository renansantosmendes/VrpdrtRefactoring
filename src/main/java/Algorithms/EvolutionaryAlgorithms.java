...
public class EvolutionaryAlgorithms {
    ...
    private static PrintStream currentAggregationStream;
    ...
    private static void initializeCurrentExecutionStreams() {
        try {
            ...
            currentExecutionOriginalParetoNormalizedStream = new PrintStream(folderName + "/" + fileName.toLowerCase()
                    + "-original-pareto-normalized-execution-" + currentExecutionNumber + ".txt");
            ...
            currentAggregationStream = new PrintStream(folderName + "/" + fileName.toLowerCase()
                    + "-current-aggregation-" + currentExecutionNumber + ".txt");
...
//                + "_" + time.getDayOfMonth();
        ...
        try {
            combinedParetoNormalizedStream = new PrintStream(folderName + "/"
                    + fileName.toLowerCase() + "-combined_pareto_normalized.csv");
    ...
    public static void saveNonDominatedSolutionsFromCurrentExecution(List<ProblemSolution> solutions) {
        ...
        currentExecutionOriginalParetoNormalizedStream.print("#\n");
        ...
        for (ProblemSolution solution : nonDominatedSolutions) {
            ...
            currentExecutionOriginalParetoNormalizedStream.print(solution.getStringWithNormalizedOriginalObjectivesForCsvFile() + "\n");
...
//                transformationList.forEach(u -> System.out.println(u));
                ...
                for (int i = 0; i < population.size(); i++) {
                    ...
                    twoPointsCrossoverForMOEAD(reducedDimension, nadirPoint, transformationList, parameters, children, population, maximumSize, probabilityOfCrossover, parents, requests,
                    ...
                    mutation2ShuffleForMOEAD(reducedDimension, nadirPoint, transformationList, parameters, child, probabilityOfMutation, requests, requestsWhichBoardsInNode,
...
//        transformationList.add(line7);
    ...
    public static String getAggregationString(List<List<Integer>> transformationList) {
        ...
        for (int i = 0; i < transformationList.size(); i++) {
            ...
            for (int j = 0; j < transformationList.get(0).size(); j++) {
                if (transformationList.get(i).get(j) == 1) {
                    int function_number = j;
                    function_number++;
                    line += "f" + function_number;
                }
            }
            aggregation.add(line);
        }
        aggregation.sort(Comparator.comparing(u -> u.charAt(1)));
        return String.join("-", aggregation);
    }
...
//                        requestsWhichBoardsInNode, requestsWhichLeavesInNode, numberOfNodes, vehicleCapacity, setOfVehicles, listOfNonAttendedRequests,
//                        requestList, loadIndexList, timeBetweenNodes, distanceBetweenNodes, timeWindows, currentTime, lastNode);
                initializeRandomPopulation(nadirPoint, parameters, reducedDimension, population, populationSize, requests,
                        requestsWhichBoardsInNode, requestsWhichLeavesInNode, numberOfNodes, vehicleCapacity, setOfVehicles, listOfNonAttendedRequests,
                        requestList, loadIndexList, timeBetweenNodes, distanceBetweenNodes, timeWindows, currentTime, lastNode);
                ...
                twoPointsCrossover(reducedDimension, nadirPoint, parameters, offspring, population, maximumSize, probabilityOfCrossover, parents, requests,
                        requestList, setOfVehicles, listOfNonAttendedRequests, requestsWhichBoardsInNode,
                        requestsWhichLeavesInNode, timeBetweenNodes, distanceBetweenNodes, numberOfNodes,
                        vehicleCapacity, timeWindows);

                mutation2Shuffle(reducedDimension, nadirPoint, parameters, offspring, probabilityOfMutation, requests, requestsWhichBoardsInNode,
                        requestsWhichLeavesInNode, numberOfNodes, vehicleCapacity, setOfVehicles, listOfNonAttendedRequests,
                        requestList, loadIndexList, timeBetweenNodes, distanceBetweenNodes, timeWindows, currentTime, lastNode);
...
//                    "adjacencies_bh_nodes_little_test", "bh_nodes_little");
    ...
    public static double[][] getMatrixOfObjetivesNormalized(List<ProblemSolution> population) {
        int rows = population.size();
        int columns = population.get(0).getOriginalObjectivesNormalized().size();
        double[][] matrix = new double[rows][columns];

        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                matrix[i][j] = population.get(i).getOriginalObjectivesNormalized().get(j);
            }
        }
        return matrix;
    }
...
//                        requestList, loadIndexList, timeBetweenNodes, distanceBetweenNodes, timeWindows, currentTime, lastNode);
                ...
                while (actualGeneration < maximumNumberOfGenerations) {
                    ...
                    onePointCrossover(reducedDimension, nadirPoint, parameters, population, file, fileSize, probabilityOfCrossover, parents, requests,
                            requestList, setOfVehicles, listOfNonAttendedRequests, requestsWhichBoardsInNode,
                            requestsWhichLeavesInNode, timeBetweenNodes, distanceBetweenNodes, numberOfNodes,
                            vehicleCapacity, timeWindows);

                    mutation2Opt(reducedDimension, nadirPoint, parameters, population, probabilityOfMutation, requests, requestsWhichBoardsInNode,
                            requestsWhichLeavesInNode, numberOfNodes, vehicleCapacity, setOfVehicles, listOfNonAttendedRequests,
                            requestList, loadIndexList, timeBetweenNodes, distanceBetweenNodes, timeWindows, currentTime, lastNode);
